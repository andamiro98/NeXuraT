        ...
        (rowId: number, field: "startDate" | "endDate", rawValue: string) => {
            setRows((prev) => {
                const nextRows = prev.map((row) => {
                    ...
                    const dur = computeDurationDays(nextRow.startDate, nextRow.endDate);
                    nextRow.durationDays = dur != null ? String(dur) : null;
                    return nextRow;
                });
                ...
                if (headerIdx !== -1) {
                    ...
                    const newRows = flattenTreeToEditableRows(roots).map((row) => {
                        ...
                        const durationDays: string | null =
                            rawDur != null && rawDur !== "" ? String(rawDur)
                                : computed != null ? String(computed)
                                    : null;
                        return { ...row, startDate, endDate, durationDays };
                    });
                    setRows(newRows);
        ...
        const handleUpdate = (ev: any) => {
            ...
            setRows((prev) => {
                const newRows = prev.map((row) => {
                    ...
                    const dur2 = computeDurationDays(nextStartDate, nextEndDate);
                    nextRow.durationDays = dur2 != null ? String(dur2) : null;
